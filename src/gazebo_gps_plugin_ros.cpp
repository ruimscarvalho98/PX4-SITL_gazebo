...
#include <gazebo_gps_plugin_ros.h>
...
namespace gazebo {
GZ_REGISTER_SENSOR_PLUGIN(GpsPluginROS)

GpsPluginROS::GpsPluginROS() : SensorPlugin()
{ }

GpsPluginROS::~GpsPluginROS()
{
...
void GpsPluginROS::Load(sensors::SensorPtr _parent, sdf::ElementPtr _sdf)
{
  ...
  if (!parentSensor_)
    gzthrow("GpsPluginROS requires a GPS Sensor as its parent");

  pub_ = ros_node_->create_publisher<sensor_msgs::msg::NavSatFix>("~/car_gps",
    qos.get_publisher_qos("~/car_gps", rclcpp::SensorDataQoS().reliable()));
  ...
  updateSensorConnection_ = parentSensor_->ConnectUpdated(boost::bind(&GpsPluginROS::OnSensorUpdate, this));
  parentSensor_->SetActive(true);

  // Listen to the update event. This event is broadcast every simulation iteration.
  updateWorldConnection_ = event::Events::ConnectWorldUpdateBegin(
      boost::bind(&GpsPluginROS::OnWorldUpdate, this, _1));
...
void GpsPluginROS::OnWorldUpdate(const common::UpdateInfo& /*_info*/)
{
...
void GpsPluginROS::OnSensorUpdate()
{
  // protect shared variables
  std::lock_guard<std::mutex> lock(data_mutex_);
  ...
  if ((current_time_ - last_gps_time_).Double() > 1 / parentSensor_->UpdateRate()) {
    ...
    uint64_t nsec = (uint64_t) gps_delay_buffer_.front().time_usec()*1e3;
    msg.header.stamp.sec = nsec / (uint64_t)1e9;
    msg.header.stamp.nanosec = (uint32_t)nsec % (uint32_t)1e9;

    msg.latitude = gps_delay_buffer_.front().latitude_deg();
    msg.longitude = gps_delay_buffer_.front().longitude_deg();
    msg.altitude = gps_delay_buffer_.front().altitude();
